export function methodCreater(methods) {
...
export function createMethodNameWithRespectiveConfig(config) {
  ...
  return function (argument = '', atRootLevel = null) {
    ...
    Object.keys(responseMockObject.defaults).forEach(key => {
      ...
      if (!responseDefaultKey.hideIfNotExist)
        responseMockObject[responseDefaultKey.field] =
          responseMockObject[responseDefaultKey.field] ||
          responseDefaultKey.defaultValue;
    });
...
function mapValueToRespone(responseMockObject, key, argument) {
  ...
  responseDefaultKey &&
    (responseMockObject[responseDefaultKey.field] =
      argument || responseDefaultKey.defaultValue);
}
